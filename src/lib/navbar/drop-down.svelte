        ...
        <ul class="primary drop-down-list list-unstyled d-flex flex-column">
            {#each menu as { func, href, icon, name, ...rest } (name)}
                {#if (rest?.menu ?? []).length !== 0}
                    {#if view === "/root"}
                        <DropDownItem func={toggle} {href} {name}>
                            <svelte:component
                                this={icon}
                                size={20}
                                slot="icon"
                    ...
                    {#if view === href}
                        ...
                        >
                            <DropDownItem href="/root" {name} func={toggle}>
                                <ArrowLeft size={20} slot="icon" />
                            </DropDownItem>

                            {#each rest.menu ?? [] as { func, icon, name }}
                                <DropDownItem {href} {func} {name}>
                                    <svelte:component
                                        this={icon}
                                        size={20}
                                        slot="icon"
                    ...
                    {/if}
                {:else if view === "/root"}
                    <DropDownItem {func} {href} {name}>
                        <svelte:component this={icon} size={20} slot="icon" />
                    </DropDownItem>
                {/if}
            {/each}
        </ul>
